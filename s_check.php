...
?>
	...
	<section id="btn2"><!--학생정보테이블쓰기버튼-->
<?php
    if (!empty($_SESSION["userId"])) {
?>
		<input type="button" class=s2_btn value="쓰기" onclick="window.open('check_add_form.php?id=<?=$id?>', 'popup', 'width=500, height=400, top=200, left=500')">
	...
	<section id="s3"><!--학생 출결,성적,시간표,수납테이블 정보-->
		<table id="table3">
			<tr>
				...
				<th style="width:170px;">출석일</th>
				<th style="width:170px;">결석일</th>
				<th style="width:50px;">요일</th> 
				<th style="width:150px;">출석시간</th>
				<th style="width:100px;">이름</th>
				<th style="width:70px;">출석</th>
				<th>사유</th>
			</tr>
<?php
	   ...
	   try {
		...
		$query = $db->query("select * 
							from checkon o, student s
							where o.id=s.id and s.id=$id");
		$cnt=1; // 번호숫자증가
...
?>
			<tr>	 
				...
				<td><?=$row["c_date"]?></td>
				<td> 
				<a href="#" onclick="window.open('check_update_form.php?id=<?=$id?>&c_datex=<?=$row["c_datex"]?>', 'popup', 'width=500, height=400, top=200, left=500')">
				<?=$row["c_datex"]?> 
				</a>
				</td> <!--[출처] http://egloos.zum.com/pdw213/v/3493140 -->
				<td><?=$row["c_week"]?></td>
				<td><?=$row["c_time"]?></td>
				<td><?=$row["name"]?></td>
				<td><?=$row["c_on"]?></td>
