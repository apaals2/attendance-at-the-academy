...
<?php
    if (!empty($_SESSION["userId"])) {
?>
		<input type="button" class=s2_btn value="쓰기" onclick="window.open('member_join_form.php', 'popup', 'width=480, height=500, top=200, left=400')">
...
?>
	...
	<?php
	   ...
	   try {
		...
		$query = $db->query("select * from grade g, course c  
							where g.grade=c.grade and g.class=c.class and g.course=c.course and g.id=$id");
		$cnt=1; // 번호숫자증가
